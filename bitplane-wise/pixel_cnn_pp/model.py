...
def model_spec(x, h=None, cond=None, init=False, reuse=False, ema=None, dropout_p=0.5, nr_resnet=5, 
               nr_filters=160, nr_logistic_mix=10, resnet_nonlinearity='concat_elu', scope=''
               ):
    """
    ...
    with arg_scope([nn.conv2d, nn.deconv2d, nn.gated_resnet, nn.dense], 
                   ...
                   reuse=reuse, dropout_p=dropout_p, prefix=scope):

        # parse resnet nonlinearity argument
        if resnet_nonlinearity == 'concat_elu':
            resnet_nonlinearity = nn.concat_elu
        elif resnet_nonlinearity == 'concat_tanh':
            resnet_nonlinearity = nn.concat_tanh
        ...
        with arg_scope([nn.gated_resnet], nonlinearity=resnet_nonlinearity, h=h, cond=cond):
...
#            x_out = (tf.nn.tanh((nn.nin(tf.nn.tanh(ul), 1))) + 1.) /2.
            
            x_out =  nn.nin(tf.nn.tanh(ul), 1)
