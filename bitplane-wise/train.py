...
#        i += 1
    ...
    if epoch % args.save_interval == 0 and epoch > 0:
        ...
        for d in test_data:
            ...
            img_tile = plotting.img_tile(outputs_scaled[:int(np.floor(np.sqrt(args.batch_size))**2)], 
                                                        aspect_ratio=1.0, border_color=1.0, stretch=True)
            img = plotting.plot_img(img_tile, title=args.data_set + ' samples')
            plotting.plt.savefig(os.path.join(args.save_dir, '%s_sample%d_iter%d.png' % (args.data_set, epoch, count)))
